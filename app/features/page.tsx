...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div style={{ border: `1px solid ${open ? C.primary : C.border}`, borderRadius: 16, overflow: 'hidden', transition: 'border-color .2s', background: C.surface }}>
      <button onClick={() => setOpen(o => !o)}
        style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left', gap: 12 }}>
        <p className="font-syne" style={{ fontSize: 15, fontWeight: 800, color: C.textDark, margin: 0 }}>{q}</p>
        <div style={{ width: 28, height: 28, borderRadius: 8, background: open ? C.primary : C.primaryLight, border: `1px solid ${open ? C.primary : C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all .2s' }}>
          {open ? <Minus size={13} style={{ color: '#ffffff' }} /> : <Plus size={13} style={{ color: C.primary }} />}
        </div>
      </button>
      {open && (
        <div style={{ padding: '0 22px 18px' }}>
          <p style={{ fontSize: 14, color: C.muted, margin: 0, lineHeight: 1.7 }}>{a}</p>
        </div>
      )}
    </div>
  )
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", backgroundColor: C.bg, minHeight: '100vh' }}>
      ...
      <div style={{ paddingTop: 72 }}>
        ...
        <div style={{ backgroundColor: C.dark, position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: -120, right: -120, width: 500, height: 500, borderRadius: '50%', background: 'rgba(117,48,251,0.15)', pointerEvents: 'none' }} />
          <div style={{ position: 'absolute', bottom: -80, left: -80, width: 320, height: 320, borderRadius: '50%', background: 'rgba(184,250,51,0.06)', pointerEvents: 'none' }} />
          <div className="max-w-5xl mx-auto px-4 md:px-6 py-18 md:py-28 text-center" style={{ position: 'relative', zIndex: 1 }}>
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full mb-6"
              style={{ backgroundColor: 'rgba(117,48,251,0.2)', border: `1px solid ${C.primary}` }}>
              <div className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: C.accent }} />
              <span className="text-[11px] font-black tracking-wider" style={{ color: C.accent }}>BUILT FOR SERIOUS EBAY SELLERS</span>
            </div>
            <h1 className="text-[34px] md:text-[58px] font-black leading-tight mb-6 font-syne" style={{ color: '#fff' }}>
              <TypewriterText
            ...
            <p className="text-[17px] leading-relaxed max-w-2xl mx-auto mb-10" style={{ color: C.textLight }}>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
              <Link href="/auth/signup"
                className="flex items-center gap-2 px-8 py-4 rounded-xl font-black text-[14px] hover:scale-105 hover:bg-[#a3e635] transition-all cursor-pointer shadow-md"
        ...
        <div style={{ backgroundColor: C.surface, borderBottom: `1px solid ${C.border}` }}>
          <div className="max-w-3xl mx-auto px-4 md:px-6 py-14 md:py-20 text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full mb-4"
              ...
              <Zap size={12} style={{ color: C.primary }} className="fill-current" />
              <span className="text-[11px] font-black tracking-wider" style={{ color: C.primary }}>LIVE ANALYSIS — COMING SOON</span>
            </div>
            <h2 className="text-[30px] font-black mb-3 font-syne" style={{ color: C.textDark }}>Try it before you sign up</h2>
            ...
            {!sandboxDone ? (
              <div style={{ background: C.bg, borderRadius: 20, padding: 24, border: `1px solid ${C.border}` }}>
                <div className="flex flex-col sm:flex-row gap-3 mb-4">
                  <input
                    ...
                    style={{ flex: 1, height: 48, padding: '0 16px', borderRadius: 12, border: `1px solid ${C.borderInput}`, fontSize: 14, color: C.text, outline: 'none', background: C.surface }}
                  />
                  <button
                    onClick={() => { }}
                    style={{ height: 48, padding: '0 20px', borderRadius: 12, background: C.dark, color: C.accent, fontSize: 13, fontWeight: 800, border: 'none', cursor: 'not-allowed', opacity: 0.7, display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <div style={{ background: C.surface, borderRadius: 14, border: `1px solid ${C.border}`, overflow: 'hidden' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', background: C.dark, borderBottom: `1px solid ${C.borderDark}` }}>
                    <span style={{ fontSize: 11, fontWeight: 800, color: C.accent }}>EXAMPLE OUTPUT — Preview only</span>
                    <span style={{ fontSize: 10, color: C.textLight }}>Real data available after signup</span>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-5">
                    {[
                      { label: 'Cassini Score', value: '94/100', color: C.primary },
                      { label: 'Est. Monthly Vol', value: '$12,450', color: C.textDark },
                      { label: 'Competition Risk', value: 'Low', color: '#16a34a' },
                      { label: 'Title Optimization', value: '87%', color: C.primary },
                    ].map((s, i) => (
                      <div key={i} style={{ textAlign: 'center', padding: 8 }}>
                        <p className="font-syne" style={{ fontSize: 22, fontWeight: 900, color: s.color, margin: '0 0 4px' }}>{s.value}</p>
                        <p style={{ fontSize: 11, fontWeight: 600, color: C.muted, margin: 0 }}>{s.label}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 mt-4">
                  <input
                    ...
                    style={{ flex: 1, height: 46, padding: '0 16px', borderRadius: 12, border: `1px solid ${C.borderInput}`, fontSize: 13, color: C.text, outline: 'none', background: C.surface }}
                  />
                  <button
                    onClick={() => sandboxEmail && setSandboxDone(true)}
                    className="hover:scale-105 transition-transform"
                    style={{ height: 46, padding: '0 22px', borderRadius: 12, background: C.accent, color: C.dark, fontSize: 13, fontWeight: 800, border: 'none', cursor: 'pointer' }}>
                    Notify me
                  </button>
                </div>
              </div>
            ) : (
              <div style={{ background: C.primaryLight, borderRadius: 20, padding: 32, border: `1px solid ${C.border}` }}>
                <Check size={32} style={{ color: C.primary, margin: '0 auto 12px' }} />
                <p className="font-syne" style={{ fontSize: 18, fontWeight: 800, color: C.textDark, margin: '0 0 4px' }}>You're on the waitlist!</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {TRUST.map((t, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div style={{ width: 34, height: 34, borderRadius: 10, background: 'rgba(117,48,251,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <t.icon size={16} style={{ color: C.accent }} />
                  </div>
                  <div>
                    <p style={{ fontSize: 12, fontWeight: 800, color: '#fff', margin: '0 0 1px' }}>{t.label}</p>
                    <p style={{ fontSize: 10, color: C.textLight, margin: 0 }}>{t.desc}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{ backgroundColor: C.surface }}>
          <div className="max-w-5xl mx-auto px-4 md:px-6 py-14 md:py-20">
            <div className="text-center mb-12">
              <p className="text-[11px] font-black tracking-wider mb-2" style={{ color: C.primary }}>THE DIFFERENCE</p>
              <h2 className="text-[26px] md:text-[36px] font-black mb-6 font-syne" style={{ color: C.textDark }}>Life with and without Riazify</h2>
              <div style={{ display: 'inline-flex', background: C.bg, borderRadius: 12, padding: 4, border: `1px solid ${C.border}`, gap: 2 }}>
                <button onClick={() => setBeforeAfter('before')}
                  style={{ padding: '8px 22px', fontSize: 13, fontWeight: 800, borderRadius: 10, border: 'none', cursor: 'pointer', background: beforeAfter === 'before' ? C.dark : 'transparent', color: beforeAfter === 'before' ? '#fff' : C.muted, transition: 'all .2s' }}>
                  Without Riazify
                </button>
                <button onClick={() => setBeforeAfter('after')}
                  style={{ padding: '8px 22px', fontSize: 13, fontWeight: 800, borderRadius: 10, border: 'none', cursor: 'pointer', background: beforeAfter === 'after' ? C.accent : 'transparent', color: beforeAfter === 'after' ? C.dark : C.muted, transition: 'all .2s' }}>
                  With Riazify
                </button>
              </div>
            </div>
              ...
              {(beforeAfter === 'before' ? [
                ...
                <FadeCard key={`${beforeAfter}-${i}`} delay={i * 45}>
                  <div style={{
                    borderRadius: 16,
                    padding: '24px 18px',
                    border: `1px solid ${beforeAfter === 'before' ? '#fee2e2' : C.border}`,
                    background: beforeAfter === 'before' ? '#fef9f9' : C.surface,
                    ...
                    <div style={{
                      ...
                      background: beforeAfter === 'before' ? '#fee2e2' : C.primaryLight,
                      border: `1px solid ${beforeAfter === 'before' ? '#fecaca' : C.border}`,
                    }}>
                      <item.icon size={20} style={{ color: beforeAfter === 'before' ? '#dc2626' : C.primary }} />
                    </div>
                    <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: '.06em', textTransform: 'uppercase', color: beforeAfter === 'before' ? '#dc2626' : C.primary }}>
                      {item.label}
                    </span>
        ...
        <div style={{ backgroundColor: C.bg }}>
          <div className="max-w-5xl mx-auto px-4 md:px-6 py-14 md:py-20">
            <p className="text-[11px] font-black tracking-wider mb-2" style={{ color: C.primary }}>ALL FEATURES</p>
            <h2 className="text-[32px] font-black mb-10 font-syne" style={{ color: C.textDark }}>Six tools. One platform.</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {FEATURES.map((f, i) => (
                <FadeCard key={f.id} delay={i * 80}>
                  <div className="transition-all duration-200 hover:-translate-y-1" style={{ background: C.surface, borderRadius: 20, padding: 24, border: `1px solid ${C.border}`, height: '100%', display: 'flex', flexDirection: 'column', gap: 16, boxShadow: '0 4px 12px rgba(117,48,251,0.03)' }}>
                    {/* Header */}
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <div style={{ width: 46, height: 46, borderRadius: 14, background: C.primaryLight, border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <f.icon size={22} style={{ color: C.primary }} />
                        </div>
                        <div>
                          <p className="font-syne" style={{ fontSize: 16, fontWeight: 900, color: C.textDark, margin: '0 0 3px' }}>{f.label}</p>
                          <span style={{ fontSize: 10, fontWeight: 800, padding: '2px 8px', borderRadius: 100, background: f.badge === 'Live' ? C.primaryLight : f.badge === 'AI Powered' ? C.accent : C.bg, color: f.badge === 'Live' ? C.primary : f.badge === 'AI Powered' ? C.dark : C.muted, border: `1px solid ${C.border}` }}>
                            {f.badge}
                          </span>
                        </div>
                      </div>
                    ...
                    {/* Content */}
                    <div style={{ flex: 1 }}>
                      <p className="font-syne" style={{ fontSize: 16, fontWeight: 800, color: C.textDark, margin: '0 0 8px', lineHeight: 1.3 }}>{f.headline}</p>
                    ...
                    {/* CTA */}
                    <Link href="/pricing"
                      className="flex items-center gap-1.5 text-[13px] font-bold transition-colors hover:text-[#6020e0]"
        ...
        <div style={{ backgroundColor: C.dark }}>
          <div className="max-w-5xl mx-auto px-4 md:px-6 py-14 md:py-20">
            <p className="text-[11px] font-black tracking-wider mb-2 text-center" style={{ color: C.accent }}>BY THE NUMBERS</p>
            <h2 className="text-[32px] font-black mb-10 text-center font-syne" style={{ color: '#fff' }}>Built for scale</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {STATS.map((s, i) => (
                <FadeCard key={i} delay={i * 100}>
                  <div className="text-center p-6 rounded-2xl" style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${C.borderDark}` }}>
                    <p className="text-[36px] font-black mb-1 font-syne" style={{ color: C.accent }}>
                    ...
                    <p className="text-[11px]" style={{ color: C.textLight }}>{s.desc}</p>
                  </div>
                </FadeCard>
              ))}
            </div>
        ...
        <div style={{ backgroundColor: C.surface }}>
          <div className="max-w-3xl mx-auto px-4 md:px-6 py-14 md:py-20">
            <div className="text-center mb-10">
              <p className="text-[11px] font-black tracking-wider mb-2" style={{ color: C.primary }}>FAQ</p>
              <h2 className="text-[26px] md:text-[34px] font-black font-syne" style={{ color: C.textDark }}>Common questions</h2>
            </div>
        ...
        <div style={{ backgroundColor: C.bg, paddingBottom: 40 }}>
          <div className="max-w-5xl mx-auto px-4 md:px-6 pt-14 md:pt-20">
            <div className="p-8 md:p-14 text-center shadow-2xl" style={{ backgroundColor: C.dark, borderRadius: 24, position: 'relative', overflow: 'hidden', border: `1px solid ${C.borderDark}` }}>
              ...
              <p className="text-[11px] font-black tracking-wider mb-4" style={{ color: C.accent }}>GET STARTED TODAY</p>
              <h2 className="text-[28px] md:text-[40px] font-black mb-3 font-syne" style={{ color: '#fff' }}>
              ...
              <div className="flex items-center justify-center gap-4 flex-wrap">
                <Link href="/auth/signup"
                  className="flex items-center gap-2 px-10 py-4 rounded-xl font-black text-[15px] hover:scale-105 hover:bg-[#a3e635] transition-all cursor-pointer shadow-lg"
                  style={{ backgroundColor: C.accent, color: C.dark }}>
                  Start free trial <ArrowRight size={16} />
                </Link>
                <Link href="/pricing"
                  className="px-8 py-4 rounded-xl font-bold text-[14px] border hover:bg-[#2d1f4e] transition-all"
                  style={{ borderColor: C.borderDark, color: '#fff' }}>
                  See pricing →
                </Link>
              </div>
