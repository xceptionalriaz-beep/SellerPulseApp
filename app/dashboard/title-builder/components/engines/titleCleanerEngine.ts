...
export class TitleCleanerEngine {
  static clean(messyTitle: string): string {
    ...
    // 1. Punctuation & emoji purge — keep letters, numbers, spaces, - / & . "
    ...
    words.forEach((word, i) => {
      ...
      // 2. Brand fixer (iphone → iPhone)
      if (BRAND_CASES[lower]) {
        cleaned.push(BRAND_CASES[lower])
      }
      // 3. Tech acronym fixer (hdmi → HDMI)
      else if (FORCE_UPPER.includes(word.toUpperCase())) {
        cleaned.push(word.toUpperCase())
      }
      // 4. Model/wattage fixer — has both digits and letters → uppercase (140w → 140W)
      else if (/\d/.test(word) && /[a-zA-Z]/.test(word)) {
        cleaned.push(word.toUpperCase())
      }
      // 5. Grammar fixer — lowercase small words UNLESS first word
