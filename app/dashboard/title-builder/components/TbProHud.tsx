...
interface Props {
  ...
  trendData: number[]
  isLoading: boolean
  topWords: { word: string; percent: string }[]
}
...
function HudRow({ icon, label, children, tooltip }: {
  ...
  return (
    <div className="flex items-start gap-2.5">
      ...
      <div className="flex items-center gap-1 shrink-0 mt-0.5" style={{ width: 105 }}>
        ...
        return (
          <span key={i} className="px-2 py-1 rounded text-[11px] font-bold text-white"
            style={{
              ...
              // Wrap long continuous strings naturally every ~60 chars,
              // show at most 2 lines (~120 chars), then ellipsis
              display: '-webkit-box',
              WebkitLineClamp: 2,
...
// ── Main TbProHud ──────────────────────────────────────────────
export default function TbProHud({ flaggedVero, currentTitle, timeframe, saturScore, trendData, isLoading, topWords }: Props) {
  const isEmpty = !currentTitle.trim()
  const isSafe = flaggedVero.length === 0
  const standby = isEmpty || isLoading

  // Fix 1: derive the worst risk level across all flagged brands so the
  // HUD icon + label reflects the most serious flag, not just a count
  ...
  const veroHudColor = isEmpty ? 'rgba(255,255,255,0.24)'
    : isSafe ? C.lime
      : hasCritical ? '#EF4444'
        : hasHighRisk ? '#FB923C'
          : '#FACC15' // Caution only — yellow
  const veroHudLabel = isEmpty ? 'Awaiting Search...'
    : isSafe ? 'SECURE (0 flags)'
      : hasCritical ? `CRITICAL BAN (${flaggedVero.length} flag${flaggedVero.length > 1 ? 's' : ''})`
        : hasHighRisk ? `HIGH RISK (${flaggedVero.length} flag${flaggedVero.length > 1 ? 's' : ''})`
          : `CAUTION (${flaggedVero.length} flag${flaggedVero.length > 1 ? 's' : ''})`
  ...
  return (
    <div className="p-5 rounded-2xl border"
      style={{
        backgroundColor: C.dark, borderColor: 'rgba(255,255,255,0.05)',
      ...
      <div className="flex flex-col gap-3">
        {/* 1. VeRO Risk — now risk-level-aware (fix 1) */}
        <HudRow
          icon={<IconBadge icon={isSafe ? ShieldCheck : AlertTriangle} color={veroHudColor} />}
          label="VeRO Risk:"
          tooltip="Account Safety Check. Shows if any brand in your title often takes down eBay listings.">
          {isEmpty
            ? <p className="text-[11px] italic" style={{ color: 'rgba(255,255,255,0.38)' }}>Awaiting Search...</p>
            : <p className="text-[13px] font-black" style={{ color: veroHudColor }}>{veroHudLabel}</p>}
        </HudRow>
        ...
        <HudRow
          ...
          {isEmpty || isLoading
            ...
            : topWords.length === 0
              ? <p className="text-[11px] italic" style={{ color: 'rgba(255,255,255,0.38)' }}>No words found</p>
              : <div className="flex flex-wrap gap-1.5">
                {topWords.slice(0, 2).map((w, i) => (
                  <WordChip key={i} word={`${i + 1}. ${w.word}`} percent={w.percent} />
                ))}
              </div>}
        </HudRow>
      </div>

      {/* Fix 6: Price Distribution — real data from the listings we already fetched.
          Replaces the blank Trend Visualizer (which required eBay's restricted
          Marketplace Insights API). The price bars come from the actual live
          competing listings returned by the eBay Browse API search. */}
      <div className="mt-5">
