            ...
            <div className="flex-1 flex flex-col min-h-0 px-6 pb-6 pt-4">
                ...
                <div className="rounded-t-2xl overflow-hidden shrink-0"
                    ...
                    <div className="flex items-center px-4"
                        ...
                        {/* Divider */}
                        <div className="w-px h-5 shrink-0 mx-2" style={{ backgroundColor: C.border }} />

                        {/* Search — auto detects EAN, eBay ID, SKU, Title */}
                        <div className="relative min-w-[200px] max-w-[280px]">
                            ...
                            <input
                                ...
                                className="w-full pl-8 pr-7 py-1.5 text-[12px] rounded-xl outline-none transition-all"
                                style={{
                                ...
                                onFocus={e => { e.target.style.borderColor = C.primary; e.target.style.boxShadow = 'none'; setShowSearchDropdown(true) }}
                                onBlur={e => { e.target.style.borderColor = C.border; e.target.style.boxShadow = 'none'; setTimeout(() => setShowSearchDropdown(false), 200) }}
                            />
                            {search && (
                            ...
                            {showSearchDropdown && recentSearches.length > 0 && !search && (
                                <div className="absolute top-full left-0 right-0 mt-1 z-50 rounded-xl overflow-hidden"
                                    style={{ backgroundColor: C.surface, border: `1px solid ${C.border}`, boxShadow: '0 8px 24px rgba(117,48,251,0.12)' }}>
                                    <p className="px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide"
                                        style={{ color: C.muted, fontFamily: 'DM Sans, sans-serif' }}>
                                        Recent Searches
                                    </p>
                                    {recentSearches.map((s, i) => (
                                        <button key={i}
                                            onClick={() => { setSearch(s); setShowSearchDropdown(false) }}
                                            className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-[#f8f7ff] transition-colors"
                                            style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 12, color: C.body }}>
                                            <Search size={11} style={{ color: C.muted }} />
                                            {s}
                                        </button>
                                    ))}
                                    <button onClick={() => setRecentSearches([])}
                                        className="w-full px-3 py-1.5 text-left text-[11px] hover:bg-[#f8f7ff] transition-colors"
                                        style={{ color: C.muted, fontFamily: 'DM Sans, sans-serif', borderTop: `1px solid ${C.border}` }}>
                        ...
                        {['Category', 'Health', 'Sort'].map(f => (
                            <button key={f}
                                className="hidden lg:flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-[12px] transition-colors hover:bg-[#f8f7ff] whitespace-nowrap"
