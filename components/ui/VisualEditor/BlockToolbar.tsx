...
export default function BlockToolbar({
    ...
    useEffect(() => {
        ...
        function handleHighlightOutside(e: MouseEvent) {
            if (highlightRef.current && !highlightRef.current.contains(e.target as Node)) {
                setShowHighlightPicker(false)
            }
        }
        document.addEventListener('mousedown', handleHighlightOutside)
    ...
    return (
        ...
        >
            ...
            {/* Live Preview toggle — eye icon only */}
            {onToggleLivePreview && (
                <button
                    ...
                    style={{
                        ...miniBtn,
                        backgroundColor: livePreview ? '#7530fb' : 'transparent',
                        color: livePreview ? '#ffffff' : '#1f1d2e',
                        borderRadius: 6,
                        width: 30,
                        height: 30,
                        border: livePreview ? '1px solid #7530fb' : '1px solid #e2e8f0',
                        flexShrink: 0,
                    }}
                >
                    {livePreview
