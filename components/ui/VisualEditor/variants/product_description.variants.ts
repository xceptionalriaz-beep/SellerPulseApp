...
function titleHtml(p: any, style: string): string {
    ...
    return `<p style="margin:0 0 14px;font-family:Arial,Helvetica,sans-serif;font-size:${p.titleFontSize ?? 16}px;font-weight:700;color:${p.titleColor ?? '#1e1535'};${style}">${p.titleText ?? 'Product Description'}</p>`
}

// Body text — shared across all variants
function bodyHtml(p: any, extraStyle = ''): string {
    return `<p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:${p.fontSize ?? 14}px;font-weight:${p.fontWeight ?? '400'};line-height:${p.lineHeight ?? 1.8};color:${p.color ?? '#6b7280'};${extraStyle}">${p.text ?? '{{ITEM_DESCRIPTION}}'}</p>`
}
    ...
    //      magazine article, not a market stall. Buyer trust goes up.
    {
        ...
        toHtml(p: any, id: string): string {
            const titleSection = p.showTitle !== false
                ? `<p style="margin:0 0 10px;font-family:Arial,Helvetica,sans-serif;font-size:${p.titleFontSize ?? 16}px;font-weight:700;color:${p.titleColor ?? '#1e1535'};padding-bottom:10px;border-bottom:1px solid #e5e7eb;">${p.titleText ?? 'Product Description'}</p>`
                : ''
...
<!--[/riazify:product_description:${id}]-->`
    ...
    //      element on the listing — creates visual coherence = trust.
    {
        ...
        toHtml(p: any, id: string): string {
            const ac = accent(p)
            const titleSection = p.showTitle !== false
                ? `<p style="margin:0 0 12px;font-family:Arial,Helvetica,sans-serif;font-size:${p.titleFontSize ?? 16}px;font-weight:700;color:${ac};">${p.titleText ?? 'Product Description'}</p>`
                : ''
            return `<!--[riazify:product_description:${id}]-->
<table width="700" cellpadding="0" cellspacing="0" border="0" align="center" style="width:100%;max-width:700px;background-color:${p.bgColor ?? '#ffffff'};">
  <tr>
    ...
    //      fills in the Properties panel — zero extra work.
    {
        ...
        toHtml(p: any, id: string): string {
          ...
          <table cellpadding="0" cellspacing="0" border="0" style="background-color:${acLight};border:1px solid ${ac}22;border-radius:20px;">
            ...
            const titleSection = p.showTitle !== false
                ? `<p style="margin:0 0 14px;font-family:Arial,Helvetica,sans-serif;font-size:${p.titleFontSize ?? 16}px;font-weight:700;color:${p.titleColor ?? '#1e1535'};">${p.titleText ?? 'Product Description'}</p>`
                : ''
            return `<!--[riazify:product_description:${id}]-->
<table width="700" cellpadding="0" cellspacing="0" border="0" align="center" style="width:100%;max-width:700px;background-color:${p.bgColor ?? '#ffffff'};">
  <!-- Feature pills row -->
...
<table width="700" cellpadding="0" cellspacing="0" border="0" align="center" style="width:100%;max-width:700px;background-color:${p.bgColor ?? '#ffffff'};">
  ...
  <tr>
    ...
    <td width="48%" style="padding:${p.showTitle !== false ? '0' : (p.paddingTop ?? 20) + 'px'} 0 ${p.paddingBottom ?? 20}px ${p.paddingLeft ?? 24}px;vertical-align:top;">
      <p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:${p.fontSize ?? 14}px;font-weight:${p.fontWeight ?? '400'};line-height:${p.lineHeight ?? 1.8};color:${p.color ?? '#6b7280'};font-style:italic;">${leftText}</p>
    </td>
    <!-- Vertical rule -->
                      ...
                      <table width="100%" cellpadding="0" cellspacing="0" border="0">
                        <tr>
                          <td style="border-bottom:2px solid ${ac};padding-bottom:10px;">
                            <p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:${p.titleFontSize ?? 16}px;font-weight:700;color:${p.titleColor ?? '#1e1535'};">${p.titleText ?? 'Product Description'}</p>
                          </td>
                        </tr>
                      </table>
                    </td>
...
<!--[/riazify:product_description:${id}]-->`
    ...
    //      while still passing eBay's accessibility requirements.
    {
        ...
        toHtml(p: any, id: string): string {
            ...
            const titleSection = p.showTitle !== false
                ? `<p style="margin:0 0 14px;font-family:Arial,Helvetica,sans-serif;font-size:${p.titleFontSize ?? 16}px;font-weight:700;color:#ffffff;letter-spacing:0.03em;text-transform:uppercase;">${p.titleText ?? 'Product Description'}</p>`
                : ''
            return `<!--[riazify:product_description:${id}]-->
