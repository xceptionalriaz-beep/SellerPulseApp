...
export default function AdminSettingsModal({ onClose }: Props) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, backgroundColor: 'rgba(30,21,53,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}
      onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ width: '100%', maxWidth: 1200, height: '88vh', backgroundColor: C.surface, borderRadius: 20, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 60px rgba(30,21,53,0.3)' }}>
              ...
              {TABS.map(tab => {
                ...
                return (
                  <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                    style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 10, border: 'none', cursor: 'pointer', textAlign: 'left', width: '100%', backgroundColor: isActive ? C.surface : 'transparent', boxShadow: isActive ? '0 1px 3px rgba(30,21,53,0.08)' : 'none', transition: 'all .15s' }}>
