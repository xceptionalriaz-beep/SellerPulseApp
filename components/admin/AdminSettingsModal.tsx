...
const TABS = [
  { id: 'brand',        label: 'Brand',       icon: Palette, desc: 'Logos, colors and brand assets'    },
  { id: 'general',      label: 'General',      icon: Globe,   desc: 'System status and configuration'  },
  { id: 'security',     label: 'Security',     icon: Shield,  desc: 'Admin security and access'        },
  { id: 'integrations', label: 'Integrations', icon: Zap,     desc: 'Third-party services and APIs'    },
]
...
export default function AdminSettingsModal({ onClose }: Props) {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, zIndex: 9999, backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}
      onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ width: '100%', maxWidth: 1000, height: '85vh', backgroundColor: C.surface, borderRadius: 20, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 60px rgba(0,0,0,0.3)' }}>
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          ...
          {/* Left sidebar */}
          <div style={{ width: 200, flexShrink: 0, borderRight: `1px solid ${C.border}`, padding: '12px 8px', display: 'flex', flexDirection: 'column', gap: 2, backgroundColor: C.bg }}>
            {TABS.map(tab => {
              ...
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  ...
                  <span style={{ fontSize: 13, fontWeight: isActive ? 700 : 500, color: isActive ? C.dark : C.muted, fontFamily: 'Inter, sans-serif' }}>{tab.label}</span>
                  {isActive && <div style={{ width: 5, height: 5, borderRadius: '50%', backgroundColor: C.lime, marginLeft: 'auto', flexShrink: 0 }}/>}
                </button>
              )
            })}
          </div>
