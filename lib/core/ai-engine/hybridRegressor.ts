...
export class HybridRegressor {
  /**
   * The "Brain" â€” calculates the future forecast line.
  ...
  static calculateForecast({
    ...
    // 1. Anchor â€” long-term slope (matches Dart _calculateLinearSlope)
    const longTermSlope = HybridRegressor._calculateLinearSlope(historicalData)

    // 2. Sprint â€” short-term velocity from last 7 data points
    ...
    // 3. Elite weighting logic â€” matches Dart momentum/stable branches
    let finalSlope: number
    if (state === MarketState.momentum) {
      // Fast-moving market â€” trust recent 7-day sprint (80% weight)
      finalSlope = shortTermSlope * 0.8 + longTermSlope * 0.2
    } else {
      // Stable market â€” trust long-term anchor (80% weight)
      finalSlope = longTermSlope * 0.8 + shortTermSlope * 0.2
    }

    // 4. Realism modifier â€” demand heat curves slope up or flattens it
    // matches Dart: heatMultiplier = 0.8 + (demandHeat * 0.4)
    const heatMultiplier = 0.8 + demandHeat * 0.4
    finalSlope *= heatMultiplier

    // 5. Generate forecast points â€” smooth daily spots
    const lastPoint = historicalData[historicalData.length - 1]
    const forecast: DataPoint[] = [lastPoint] // start from today

    // Anti-crash ceiling â€” matches Dart maxHistoricalY * 2.5
