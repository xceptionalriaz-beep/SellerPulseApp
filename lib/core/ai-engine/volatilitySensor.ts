...
// ── MarketState enum (matches Dart enum MarketState) ─────────
...
export class VolatilitySensor {
  ...
  static detectState(spots: DataPoint[]): MarketState {
    ...
    // 1. Mean (average sales) — matches Dart sumY / spots.length
    const sumY = spots.reduce((sum, s) => sum + s.y, 0)
    const mean = sumY / spots.length

    if (mean === 0) return MarketState.uncertain

    // 2. Standard deviation — matches Dart math.sqrt(sumSquaredDiff / spots.length)
    ...
    // 3. Coefficient of Variation (CV) — matches Dart cv = standardDeviation / mean
    const cv = standardDeviation / mean

    // 4. Brain switching logic — matches Dart if/else
